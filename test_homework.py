...
def test_dark_theme_by_time_and_user_choice():
    ...
    is_dark_theme = (not (6 <= current_time.hour <= 22) if dark_theme_enabled_by_user is None
                     else dark_theme_enabled_by_user)
    assert is_dark_theme is True
...
def print_function_info(func, *args):
    """ Печатает читаемое имя переданной ей функции и значений аргументов. """
    func_name = func.__name__.replace('_', ' ').title()
    args_list = ', '.join(args)
    print(f"{func_name} [{args_list}]")
    return f"{func_name} [{args_list}]"
